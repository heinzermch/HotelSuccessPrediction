...
for(j in 1:nb_cross_validation){
  ...
  for (i in 1:nb_imputations){
    ...
    # Transform revenue to RevPAR
    orig_train_revenue <- train_hotels$revenue
    orig_test_revenue <- test_hotels$revenue
    train_hotels$revenue <- train_hotels$revenue / 360 / train_hotels$rooms
    test_hotels$revenue <- test_hotels$revenue / 360 / test_hotels$rooms
    ...
    # Prepare data for xgbBoost
    xgb_train_hotels <- train_hotels[ , !(names(train_hotels) %in% c("revenue"))]
    xgb_train_revenue <- subset(train_hotels, select = c("revenue"))
    
    xgb_test_hotels <- test_hotels[ , !(names(test_hotels) %in% c("revenue"))]
    
    
    xgb_train_hotels <- as.matrix(xgb_train_hotels)
    xgb_train_revenue <- as.matrix(xgb_train_revenue)
    xgb_test_hotels <- as.matrix(xgb_test_hotels)
    
    # run model
    revenueFit <- xgboost(data = xgb_train_hotels, label = xgb_train_revenue, max.depth = param_depth, eta = param_eta, nthread = 2, nround = param_nb_steps, objective = "reg:linear", verbose = 1)
    ...
    test_prediction <- predict(revenueFit, newdata = xgb_test_hotels)
    # fiddle: only to calculate model fit for test sample
    train_prediction <- predict(revenueFit, newdata = xgb_train_hotels)
    ...
    # Compare to simple median guess
    ...
    # Restore the original revenue values
    train_hotels$revenue <- orig_train_revenue
    test_hotels$revenue <- orig_test_revenue
  }
}
...
# Make a basic graph
plot( b~x , type="b" , bty="l" , xlab="Dataset" , ylab="RMSE/RevPAR" , col=rgb(0.2,0.4,0.1,0.7) , lwd=3 , pch=17 , ylim=c(0,500), main ="RMSE for RevPAR Estimation by Boosted Trees")
lines(c ~x , col=rgb(0.8,0.4,0.1,0.7) , lwd=3 , pch=19 , type="b" )
lines(d ~x , col=rgb(0.2,0.1,0.8,0.7) , lwd=3 , pch=15 , type="b" )

# Add a legend
legend("bottomleft", 
       legend = c("RMSE Boosted Trees", "RMSE Mean Prediction", "Mean RevPAR"), 
...
# Make a basic graph
plot( b~x , type="b" , bty="l" , xlab="Dataset" , ylab="MAE/RevPAR" , col=rgb(0.2,0.4,0.1,0.7) , lwd=3 , pch=17 , ylim=c(0,300), main ="MAE for RevPAR Estimation by Boosted Trees")
lines(c ~x , col=rgb(0.8,0.4,0.1,0.7) , lwd=3 , pch=19 , type="b" )
lines(d ~x , col=rgb(0.2,0.1,0.8,0.7) , lwd=3 , pch=15 , type="b" )

# Add a legend
legend("bottomleft", 
       legend = c("MAE Boosted Trees", "MAE Mean Prediction", "Mean RevPAR"), 
       col = c(rgb(0.2,0.4,0.1,0.7), 
               rgb(0.8,0.4,0.1,0.7), rgb(0.2,0.1,0.8,0.7)), 
       pch = c(17,19,15), 
       bty = "n", 
...
colnames(train_hotels)

# Variable importance plot
train_hotels <- train_hotels[ , !(names(train_hotels) %in% c("revenue"))]
importance <- xgb.importance(feature_names = colnames(train_hotels), model = revenueFit)
head(importance)
xgb.plot.importance(importance_matrix = importance, main = "Variable Importance for Boosted Trees RevPAR Estimation")
